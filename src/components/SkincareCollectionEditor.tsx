...
import { 
  getSkincareCollectionConfig, 
  saveSkincareCollectionConfig,
  resetAdminStoreToDefaults
} from '../lib/adminStore';
import { SkincareCollectionConfig } from '../types';
import { 
  Check, 
  RefreshCw, 
  ShoppingBag
...
export default function SkincareCollectionEditor() {
  const [config, setConfig] = useState<SkincareCollectionConfig | null>(null);
  ...
  const handleSave = () => {
    if (!config) return;
    saveSkincareCollectionConfig(config);
    setSaveSuccess(true);
    setTimeout(() => setSaveSuccess(false), 3000);
  };

  const handleReset = () => {
    if (confirm("Are you sure you want to restore default Skincare Collection settings?")) {
      resetAdminStoreToDefaults();
      setConfig(getSkincareCollectionConfig());
      setSaveSuccess(true);
      setTimeout(() => setSaveSuccess(false), 3000);
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-150">
          <div>
            <h3 className="font-serif font-bold text-xl text-[#003334] flex items-center gap-2">
              ...
              <span>Skincare Collection Editor</span>
            </h3>
            <p className="text-xs text-slate-500">
              Manage the skincare products section displayed on the homepage.
            </p>
          </div>
          <button
            onClick={handleReset}
        ...
        {/* Note: Product editing would require a larger UI, keeping it simple for now */}
        <div className="pt-4 border-t border-slate-150">
           <p className="text-xs text-slate-500 italic">Products are currently managed via the data source. Edit the section text above.</p>
        </div>
