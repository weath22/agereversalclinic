...
export default function FooterEditor() {
  ...
  const handleReset = () => {
    if (confirm("Are you sure you want to restore default Footer settings?")) {
      resetAdminStoreToDefaults();
      setConfig(getFooterConfig());
      setSaveSuccess(true);
      setTimeout(() => setSaveSuccess(false), 3000);
  ...
  const handleLinkFieldChange = (index: number, field: 'name' | 'href', value: string) => {
    setConfig({
      ...config,
    ...
    <div className="space-y-8 animate-fadeIn">
      <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-150">
          <div>
            <h3 className="font-serif font-bold text-xl text-[#003334] flex items-center gap-2">
              <PanelBottom className="h-5 w-5 text-rose-gold" />
              <span>Footer Editor</span>
            </h3>
            <p className="text-xs text-slate-500">
              Manage the website footer information and links.
            </p>
          </div>
          <button
            onClick={handleReset}
        ...
        <div className="pt-4 border-t border-slate-150">
          <h4 className="font-serif font-bold text-base text-[#003334] mb-4">Social Links</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className="space-y-2">
               <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Facebook URL</label>
               <input
                 type="text"
                 className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#003334] focus:border-transparent text-sm text-slate-800 font-medium"
             ...
             <div className="space-y-2">
               <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Instagram URL</label>
               <input
                 type="text"
                 className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#003334] focus:border-transparent text-sm text-slate-800 font-medium"
             ...
             <div className="space-y-2">
               <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Twitter URL</label>
               <input
                 type="text"
                 className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#003334] focus:border-transparent text-sm text-slate-800 font-medium"
             ...
             <div className="space-y-2">
               <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Youtube URL</label>
               <input
                 type="text"
                 className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#003334] focus:border-transparent text-sm text-slate-800 font-medium"
        ...
        <div className="space-y-6 pt-4 border-t border-slate-150">
          <div className="flex items-center justify-between">
            <h4 className="font-serif font-bold text-base text-[#003334]">Navigation Links ({config.links.length})</h4>
            <button
              onClick={addLink}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {config.links.map((link, index) => (
              <div key={index} className="bg-slate-50/50 p-5 rounded-2xl border border-slate-200/80 space-y-4 relative flex flex-col justify-between">
                <div>
                  ...
                  <div className="space-y-3.5">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        value={link.name}
                        onChange={(e) => handleLinkFieldChange(index, 'name', e.target.value)}
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-400 uppercase block">URL (href)</label>
                      <input
                        type="text"
                        className="w-full px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-bold text-slate-800"
                        value={link.href}
                        onChange={(e) => handleLinkFieldChange(index, 'href', e.target.value)}
                      />
                    </div>
                  </div>
                </div>
