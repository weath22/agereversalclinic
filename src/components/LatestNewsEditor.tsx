...
import { 
  getLatestNewsConfig, 
  saveLatestNewsConfig,
  resetAdminStoreToDefaults
} from '../lib/adminStore';
import { LatestNewsConfig } from '../types';
import { 
  Check, 
  RefreshCw, 
  Newspaper
...
export default function LatestNewsEditor() {
  const [config, setConfig] = useState<LatestNewsConfig | null>(null);
  ...
  const handleSave = () => {
    if (!config) return;
    saveLatestNewsConfig(config);
    setSaveSuccess(true);
    setTimeout(() => setSaveSuccess(false), 3000);
  };

  const handleReset = () => {
    if (confirm("Are you sure you want to restore default Latest News settings?")) {
      resetAdminStoreToDefaults();
      setConfig(getLatestNewsConfig());
      setSaveSuccess(true);
      setTimeout(() => setSaveSuccess(false), 3000);
    ...
    <div className="space-y-8 animate-fadeIn">
      <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-150">
          <div>
            <h3 className="font-serif font-bold text-xl text-[#003334] flex items-center gap-2">
              <Newspaper className="h-5 w-5 text-rose-gold" />
              <span>Latest News Editor</span>
            </h3>
            <p className="text-xs text-slate-500">
              Manage the news and insights section displayed on the homepage.
            </p>
          </div>
          <button
            onClick={handleReset}
        ...
        <div className="pt-4 border-t border-slate-150">
           <p className="text-xs text-slate-500 italic">Articles are currently managed via the data source. Edit the section text above.</p>
        </div>
