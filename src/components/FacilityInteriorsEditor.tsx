...
import { 
  getFacilityInteriorsConfig, 
  saveFacilityInteriorsConfig,
  resetAdminStoreToDefaults
} from '../lib/adminStore';
import { FacilityInteriorsConfig } from '../types';
import { 
  Check, 
  RefreshCw, 
  Home, 
...
export default function FacilityInteriorsEditor() {
  const [config, setConfig] = useState<FacilityInteriorsConfig | null>(null);
  const [saveSuccess, setSaveSuccess] = useState(false);

  useEffect(() => {
    setConfig(getFacilityInteriorsConfig());
  }, []);

  const handleSave = () => {
    if (!config) return;
    saveFacilityInteriorsConfig(config);
    setSaveSuccess(true);
    setTimeout(() => setSaveSuccess(false), 3000);
  };

  const handleReset = () => {
    if (confirm("Are you sure you want to restore default Facility settings?")) {
      resetAdminStoreToDefaults();
      setConfig(getFacilityInteriorsConfig());
      setSaveSuccess(true);
      setTimeout(() => setSaveSuccess(false), 3000);
  ...
  const deleteRoom = (roomId: string) => {
    setConfig({
      ...config,
      rooms: config.rooms.filter(r => r.id !== roomId)
    });
  };

  const handleRoomFieldChange = (roomId: string, field: 'title' | 'tag' | 'image', value: string) => {
    setConfig({
      ...config,
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-150">
          <div>
            <h3 className="font-serif font-bold text-xl text-[#003334] flex items-center gap-2">
              <Home className="h-5 w-5 text-rose-gold" />
              <span>Facility Interiors Editor</span>
            </h3>
            <p className="text-xs text-slate-500">
              Manage the facility images and rooms showcased on the homepage.
            </p>
          </div>
          <button
            onClick={handleReset}
        ...
        <div className="space-y-6 pt-4 border-t border-slate-150">
          <div className="flex items-center justify-between">
            <h4 className="font-serif font-bold text-base text-[#003334]">Rooms ({config.rooms.length})</h4>
            <button
              onClick={addRoom}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {config.rooms.map((room, index) => (
              <div key={room.id} className="bg-slate-50/50 p-5 rounded-2xl border border-slate-200/80 space-y-4 relative flex flex-col justify-between">
                <div>
                  ...
                  <div className="space-y-3.5">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        value={room.title}
                        onChange={(e) => handleRoomFieldChange(room.id, 'title', e.target.value)}
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-400 uppercase block">Tag (e.g. Private, Clinical)</label>
                      <input
                        type="text"
                        className="w-full px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-medium text-slate-600"
                        value={room.tag}
                        onChange={(e) => handleRoomFieldChange(room.id, 'tag', e.target.value)}
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        value={room.image}
                        onChange={(e) => handleRoomFieldChange(room.id, 'image', e.target.value)}
                      />
