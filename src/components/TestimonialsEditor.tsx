...
export default function TestimonialsEditor() {
  ...
  const handleReset = () => {
    if (confirm("Are you sure you want to restore default Testimonials settings?")) {
      resetAdminStoreToDefaults();
      setConfig(getTestimonialsConfig());
      setSaveSuccess(true);
      setTimeout(() => setSaveSuccess(false), 3000);
    ...
    setConfig({
      ...
      testimonials: [
        ...config.testimonials,
        {
          ...
          rating: 5,
          quote: 'Excellent service and care.'
        }
      ]
    });
  };
  ...
  const deleteTestimonial = (testId: string) => {
    setConfig({
      ...config,
      testimonials: config.testimonials.filter(t => t.id !== testId)
    });
  };

  const handleTestimonialFieldChange = (testId: string, field: 'name' | 'role' | 'image' | 'quote' | 'rating', value: string | number) => {
    setConfig({
      ...config,
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-150">
          <div>
            <h3 className="font-serif font-bold text-xl text-[#003334] flex items-center gap-2">
              <MessageSquareQuote className="h-5 w-5 text-rose-gold" />
        ...
        <div className="space-y-6 pt-4 border-t border-slate-150">
          <div className="flex items-center justify-between">
            <h4 className="font-serif font-bold text-base text-[#003334]">Testimonials ({config.testimonials.length})</h4>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {config.testimonials.map((test, index) => (
              <div key={test.id} className="bg-slate-50/50 p-5 rounded-2xl border border-slate-200/80 space-y-4 relative flex flex-col justify-between">
                <div>
                  ...
                  <div className="space-y-3.5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          value={test.role}
                          onChange={(e) => handleTestimonialFieldChange(test.id, 'role', e.target.value)}
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-slate-400 uppercase block">Rating (1-5)</label>
                        <input
                          type="number"
                          min="1"
                          max="5"
                          className="w-full px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-medium text-slate-600"
                          value={test.rating}
                          onChange={(e) => handleTestimonialFieldChange(test.id, 'rating', Number(e.target.value))}
                        />
                      </div>
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-400 uppercase block">Quote</label>
                      <textarea
                        rows={3}
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        value={test.image}
                        onChange={(e) => handleTestimonialFieldChange(test.id, 'image', e.target.value)}
                      />
                    </div>
                  </div>
                </div>
